...
import org.example.funtions.WaterSensorMapFunction;
...
public class WindowApiDemo {
    public static void main(String[] args) throws Exception {
        ...
        //TODO 1. 指定 窗口分配器： 指定用 哪一种窗口 --- 时间 or 计数？ 滚动、滑动、会话？
...
//        sensorKS.window(ProcessingTimeSessionWindows.withGap(Time.seconds(5)));//会话窗口，时间间隔5s

        // 基于计数
//        sensorKS.countWindow(5);//滚动窗口，窗口长度=5个元素
//        sensorKS.countWindow(5, 2);//滑动窗口，窗口长度=5个元素，滑动步长=2个元素
//        sensorKS.window(GlobalWindows.create());//全局窗口，计数窗口的底层就是用的这个，需要自定义的时候才会用(驱逐器，触发器)
